...
layui.define(["jquery","laytpl"], function (exports) {
    ...
    window.getLayerOpenIndex = function() {
    	return layerOpenIndex;
    }
...
//                ,$.inArray("remove",options.contextmenuList)!==-1?'<li class="remove"><a href="javascript:;">删除</a></li>':''
            	...
            	,$.inArray("addNode",options.contextmenuList)!==-1?'<li class="addNode"><a href="javascript:;">添加</a></li>':''
            	,$.inArray("delNode",options.contextmenuList)!==-1?'<li class="delNode"><a href="javascript:;">删除</a></li>':''
            	,$.inArray("impNode",options.contextmenuList)!==-1?'<li class="impNode"><a href="javascript:;">导入</a></li>':''
            	,$.inArray("expNode",options.contextmenuList)!==-1?'<li class="expNode"><a href="javascript:;">导出</a></li>':''
            ,'</ul>'].join("");
            ...
            options.elem.off("contextmenu").on("contextmenu",".eleTree-node-content",function(e) {
                ...
                // 自定义添加
                $("#tree-menu li.addNode").off().on("click",function() {
                	var node = $(that).parent(".eleTree-node");
                	var path = node.attr("data-" + options.request.key);
                	layer.open({
                		type: 2,
                		title: '添加',
                		resize: false,
                		...
                		success: function (layero, index) {
                			 layerOpenIndex = index;
                		}
            		});
                });
                // 自定义删除
                $("#tree-menu li.delNode").off().on("click",function() {
                	var node = $(that).parent(".eleTree-node");
                	var path = node.attr("data-" + options.request.key);
                	layer.open({
                		type: 2,
                ...
                // 自定义导入
                $("#tree-menu li.impNode").off().on("click",function() {
                	var node = $(that).parent(".eleTree-node");
                	var path = node.attr("data-" + options.request.key);
                	layer.open({
                		type: 2,
                ...
                // 自定义导出
                $("#tree-menu li.expNode").off().on("click",function() {
                	var node = $(that).parent(".eleTree-node");
                	var path = node.attr("data-" + options.request.key);
                	layer.open({
                		type: 2,
